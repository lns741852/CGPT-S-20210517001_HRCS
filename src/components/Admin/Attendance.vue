<template>
  <div class="Attendance">
    <div class="top">
      <div class="left">
        <div class="leftBar">
          <button @click="inputWindowswitch()" class="btnAdd">自檔案匯入班表</button>
    ...
    <transition name="timeWintran">
      <div class="timeWindow" v-show="timeWindow.timeWindowswitch">
        <div class="timeWindow_content">
          <div class="title_box">
            <span>時段總覽</span>
            <button class="btn left_window_btn" @click="addTimedata()">新增</button>
            <button class="btn close_window_btn" @click="timeSwitch()">&times;</button>
          </div>
        ...
        <div class="timeEditwindow_content">
          ...
          <div>
            <form @submit.prevent="saveTimedata">
              <div class="addForm_box">
                ...
                <div class="addForm_row">
                  ...
                  <div class="addForm_input">
                    ...
                    <div class="addForm_lable_error">{{ timeDataerror.endTime }}</div>
                  </div>
                </div>
                <div class="addForm_row">
              ...
              <button type="submit" class="btn timeEditwindow_submit">儲存</button>
            </form>
          </div>
        </div>
      </div>
...
let fileName = "";
let importData = [];
const ckDate = /^(?:(?:1[6-9]|[2-9][0-9])[0-9]{2}([-/.]?)(?:(?:0?[1-9]|1[0-2])\1(?:0?[1-9]|1[0-9]|2[0-8])|(?:0?[13-9]|1[0-2])\1(?:29|30)|(?:0?[13578]|1[02])\1(?:31))|(?:(?:1[6-9]|[2-9][0-9])(?:0[48]|[2468][048]|[13579][26])|(?:16|[2468][048]|[3579][26])00)([-/.]?)0?2\2(?:29))$/;
...
export default {
    ...
    return {
      ...
      table_Options: {
        ...
        childRow: function (h, row) {
          ...
          mapSrc =
            "https://www.google.com/maps/embed/v1/place?key=AIzaSyBxUw32LsyDvJPOnXf2zLazB6hCs7nZYUc&q=" +
            local_Position +
            "&language=zh-TW";
  ...
  methods: {
    ...
    async editTimeswitch() {
      this.timeEditwindow.timeEditwindowswitch = !this.timeEditwindow
        .timeEditwindowswitch;
    },
    /**搜尋*/
    search() {
    ...
    /** 匯入班表員工編號篩選 */
    queryEMP() {
      Event.$emit("vue-tables.filter::empFilter", this.importWindow.selectImportEMP);
    },
    /**enter事件*/
    enter(event) {
      if (event.keyCode === 13) {
      ...
      if (attend_Data.attend_no) {
        if (attend_Data.punch_out || attend_Data.punch_in) {
          if (confirm(delMSG + "員工姓名：" + attend_Data.emp_name)) {
            let resData = await userService.deleteAttendPunch(attend_Data.attend_no);
            if (
      ...
      for (const key in jsonData) {
        if (Object.hasOwnProperty.call(jsonData, key)) {
          ...
          if (key == 0) {
            ...
            rowEmpno = row[1];
            rowText =
              row[rowMaxlength - 1] != null && row[rowMaxlength - 1] != undefined
                ? row[rowMaxlength - 1]
                : " ";
            ...
            row.forEach((code, index) => {
              if (code != undefined && code.length == 1 && index != rowMaxlength - 1) {
                tempRowdata.push({ code, index });
              }
            });
            tempRowdata.forEach((rowTime) => {
              let rowDate = dateData[rowTime.index - 3];
              let rowShiftno = "SH" + rowEmpno + moment(rowDate).format("YYYYMMDD");
              ...
              if (rowTime.startTime != undefined) {
                ...
                hoursData = moment(rowClockout).diff(moment(rowClockin), "hours");
    ...
    async insertData() {
      ...
      importData.forEach(async (schedule) => {
          ...
          if (msgList.length != 0) {
            msgList.forEach((err) => {
              ...
              if (err.MSG.msg.indexOf("此編號已有資料") != -1) {
                ...
                msgText += err.schedule.shift_no + "此編號發生錯誤 " + err.MSG.msg + "\n";
              }
            });
          }
          alert("上傳完成!");
